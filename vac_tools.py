...
class DisplacementTensor:
    ...
    def autocorr(self):
        ...
        PSDxx = Fx.conjugate()*Fx
        PSDxy = 0.5*(Fx.conjugate()*Fy + Fy.conjugate()*Fx)
        PSDyy = Fy.conjugate()*Fy
    ...
    def displacementTensor(self):
        ...
        disp_tens[:, 0, 0] = 1/N_particles*(S2xx.sum(axis=1))
        disp_tens[:, 0, 1] = 1/N_particles*(S2xy.sum(axis=1))
        disp_tens[:, 1, 1] = 1/N_particles*(S2yy.sum(axis=1))
